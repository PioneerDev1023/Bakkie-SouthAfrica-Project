$(document).ready(function(){
    ...
    $.get("./get_userdata.php", function(data, status){
        ...
        for(var i = 0; i < obj_len; i++){
            ...
            // $newTaskTemplate.find('#user_photo').val(show_data[i]['user_photo']);
            // console.log(show_data[i]['user_photo']);
            $newTaskTemplate.find('#nickname_new').val(show_data[i]['nickname']);
            ...
            $newTaskTemplate.find('.user-img-new').attr('src', './../../RU/inc' + show_data[i]['user_photo']);
            $newTaskTemplate.find('.bakkie-img').attr('src', './../../RU/inc' + show_data[i]['bakkie_photo']);
            ...
            $newTaskTemplate.find('#form-content').html("<button onclick='displayDate(`" + show_data[i]['email'] + "`)' style='margin-right: 10px' class='btn btn-primary approve-btn'>Approve</button><button onclick='displayDate1(`" + show_data[i]['email'] + "`)' class='btn btn-danger disapprove-btn'>Disapprove</button>");
