$(document).ready(function(){
    ...
    $.get("./get_alluserdata.php", function(data, status){
        var show_data = JSON.parse(data);
        var obj_len = (Object.keys(show_data).length);
        for(var i = 0; i < obj_len; i++){
            const $newTaskTemplate = $taskTemplate.clone();
            ...
            $newTaskTemplate.find('#form-content').html("<button onclick='displayDate(`" + show_data[i]['email'] + "`)' style='margin-right: 10px; display: none;' class='btn btn-primary approve-btn'z>Approve</button><button onclick='displayDate1(`" + show_data[i]['email'] + "`)' class='btn btn-danger disapprove-btn'>Delete</button>");
